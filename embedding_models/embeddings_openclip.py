...
def get_embedding(text: str) -> list[float]:
    """Compute a single text embedding via OpenCLIP, normalized to unit length."""
    if not text:
        return []
...
def get_embeddings(texts: list[str]) -> list[list[float]]:
    """Compute OpenCLIP embeddings for a list of texts."""
    if not texts:
        return []
...
def get_similar_classes_openclip(candidates_classes: list[str],
